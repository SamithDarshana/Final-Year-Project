...
#         self.latest = {"eye_state": None, "yawning": None, "emotion": None}
#         self.last_eye_closed_time = None
#         self.EYE_CLOSED_THRESHOLD = 1  # seconds
...
#                                 cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255) if yawning_state == "yes" else (255, 255, 255), 2)
#                     break  # only process first face

#                 # === Update latest detection state ===
#                 with self.lock:
#                     self.latest = {
#                         "timestamp": datetime.now().isoformat(),
#                         "eye_state": eye_state,
...
#         print(
#             f"🎥 Detection started for {self.session_data.get('student_name')}")
#         print(f"⚙ Current thresholds: EYE_CLOSED_THRESHOLD={self.EYE_CLOSED_THRESHOLD}s, "
...
#                     # Show alert text if drowsiness triggered
...
#     try:
#         time.sleep(5)
#         # Simulate API call: stricter mode
                    ...
                    for (ex, ey, ew, eh) in eyes[:1]:
                        ...
                        if eye_state == "closed":
                            if self.last_eye_closed_time is None:
                                ...
                                self.session_data["drowsiness_alerts"] += 1
                                self.last_eye_closed_time = None
                                # self._save_snapshot(frame, "drowsiness_alert")
                            self.session_data["eye_closures"] += 1
                        else:
                            self.last_eye_closed_time = None
...
#             "models/Eye-State-Detection-DenseNet121.h5")
#         self.yawn_model = load_model("models/yawn_detector_model.h5")
#         # load_model("models/model.h5")
#         self.emotion_model = load_model(
#             "models/emotion_detector_model.h5", compile=False)
...
#                     # === Emotion Detection ===
#                     try:
#                         face_input = self._preprocess_face(roi_color)
#                         emotion_pred = self.emotion_model.predict(
#                             face_input, verbose=0)
#                         emotion_state = self.emotion_labels[np.argmax(
#                             emotion_pred)]
...
#     # === Preprocessing helpers (All resized to 224x224 RGB) ===
#     def _preprocess_eye(self, eye_img):
#         eye_img = cv2.resize(eye_img, (224, 224))
...
#     def _preprocess_mouth(self, mouth_img):
#         mouth_img = cv2.resize(mouth_img, (56, 58))
